...
fn table_set(martix: &mut Grid, i: usize, j: usize, val: usize) {
    match martix.set(i, j, val) {
...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    let n = seq2.len();
    let mut lcs_table: Grid = Grid::new(m + 1, n + 1);
    for i in 0..m + 1 {
        table_set(&mut lcs_table, i, 0, 0);
    }
    for j in 0..n + 1 {
        table_set(&mut lcs_table, 0, j, 0);
    }
    for i in 0..m {
        for j in 0..n {
            if seq1[i] == seq2[j] {
                let tmp_val = lcs_table.get(i, j).unwrap();
                table_set(&mut lcs_table, i + 1, j + 1, tmp_val + 1);
            } else {
                let tmp_val = max(
                    lcs_table.get(i + 1, j).unwrap(),
                    lcs_table.get(i, j + 1).unwrap(),
                );
                table_set(&mut lcs_table, i + 1, j + 1, tmp_val);
            }
        }
    }
    return lcs_table;
...
fn main() {
    ...
    // unimplemented!();
    // Be sure to delete the #[allow(unused)] line above
    let lines1: Vec<String> = read_file(filename1);
    let lines2: Vec<String> = read_file(filename2);
